...
function RepositoryPanel({ repo, limits, refresh }: { repo: LocalRepository; limits: Limits; refresh: () => void }) {
  ...
  return <section><div className="local-heading"><div><p className="local-eyebrow">{repo.source_kind === 'github' ? 'PUBLIC GITHUB REPOSITORY' : 'LOCAL SOURCE SNAPSHOT'}</p><h1>{repo.name}</h1><p className="local-muted">{repo.commit_sha ? `Git commit ${repo.commit_sha.slice(0, 12)}` : `Content ${repo.snapshot_id.slice(6, 18)}`} · Saved on this computer</p></div><button className="local-primary" disabled={busy || loading} onClick={async () => { setStarting(true); setError(''); try { setJob(await request('analyze', { repository_id: repo.repository_id, snapshot_id: repo.snapshot_id })); } catch (error) { setError(message(error)); } finally { setStarting(false); } }}>{busy ? 'Analyzing…' : report ? 'Analyze snapshot' : 'Run analysis'}</button></div>
    ...
    <div className="local-report" key={view}><div className="local-section-heading"><h2>{view}</h2><span className="local-section-note">Snapshot evidence</span></div>{loading ? <p role="status">Loading saved sources…</p> : view === 'Files' ? <>
      ...
      <main id="main-content" tabIndex={-1}>
        ...
        {!limits ? <div className="local-loading" role="status"><span className="local-spinner"/>Opening your workspace…</div> : repo ? <RepositoryPanel key={`${repo.repository_id}:${repo.snapshot_id}`} repo={repo} limits={limits} refresh={() => { void refresh().catch(error => setError(message(error))); }}/> : <>
          <div className="local-hero">
            <div><h1>Your code,<br/><span>untangled.</span></h1><p>See the architecture.<br className="local-mobile-break"/> Follow the connections.</p></div>
            <svg className="local-thread" viewBox="0 0 480 250" fill="none" aria-hidden="true"><path d="M0 210C100 210 40 55 165 70S335 210 375 118 235 60 310 25 420 70 480 0"/><path d="M0 235C110 215 72 85 168 93S340 235 400 130 260 75 330 35 420 85 480 20"/></svg>
          </div>
          <div className="local-import-grid">
            <section className="local-import-section"><div className="local-import-title"><span className="local-icon-tile"><LocalIcon name="branch"/></span><div><h2>Import a public repository</h2><p>Start with a GitHub link.</p></div></div>
              <form onSubmit={async event => { event.preventDefault(); setBusy(true); setError(''); try { const imported = repositorySchema.parse(await request('github', { github_url: url })); window.location.assign(`/?${new URLSearchParams({repo: imported.repository_id})}`); } catch (error) { setError(message(error)); } finally { setBusy(false); } }}>
                <label htmlFor="github-url">GitHub repository URL</label><input id="github-url" type="url" placeholder="https://github.com/owner/repository" required value={url} onChange={event => setUrl(event.target.value)}/>
                <button className="local-primary" disabled={busy}>{busy ? <><span className="local-spinner"/>Importing repository…</> : <>Import repository<LocalIcon name="arrow"/></>}</button>
              </form>
            </section>
            <section className="local-import-section"><div className="local-import-title"><span className="local-icon-tile"><LocalIcon name="folder"/></span><div><h2>Open a local source folder</h2><p>Your private code. Your workspace.</p></div></div><FolderImport limits={limits} imported={() => { void refresh().catch(error => setError(message(error))); }}/></section>
          </div>
          <section className="local-library" aria-labelledby="repository-list-title">
            ...
            </a>)}</div> : <div className="local-empty"><LocalIcon name={filter ? 'search' : 'folder'}/><h3>{filter ? 'No matching repositories' : 'A clearer picture starts here.'}</h3><p>{filter ? 'Try a different repository name.' : 'Import a GitHub repository or choose a source folder above.'}</p>{filter && <button className="local-secondary" onClick={() => setFilter('')}>Clear search</button>}</div>}
          </section>
        </>}
      </main>
      <footer className="local-footer"><span>Ariadne <span className="local-footer-divider">/</span> Source-first understanding</span><span>Made for curious developers.</span></footer>
    </div>
  </div>;
}
